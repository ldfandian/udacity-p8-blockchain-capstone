...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0), 'the new owner is a not real address');
...
contract Pausable is Ownable {
    ...
    //  4) create 'whenNotPaused' & 'whenPaused' modifier that throws in the appropriate situation
    ...
    function resume() public onlyOwner whenPaused {
        _paused = false;
        emit ContractUnpaused(getOwner());
    }
}
...
contract ERC721 is Pausable, ERC165 {
    ...
    function ownerOf(uint256 tokenId) public view returns (address) {
        ...
        require (tokenOwner != address(0), 'the token does not exist');
        return tokenOwner;
    }
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        address tokenOwner = _tokenOwner[tokenId];
        require(to != tokenOwner, 'the to address cannot be the owner of the token');
        ...
        require((msg.sender == tokenOwner) || _operatorApprovals[tokenOwner][msg.sender],
            'require the owner of the contract or being approved by the owner');
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        ...
        require(approval != address(0), 'the token approval does not exist');
        return approval;
    }
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(!_exists(tokenId), 'the given token already exists');
        require(to != address(0), 'to address is invalid');
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        address tokenOwner = _tokenOwner[tokenId];
        require(from == tokenOwner, 'from address is not the owner of the given token');
        require(to != tokenOwner, 'cannot transfer to the same address');
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    constructor (string memory name, string memory symbol, string memory baseTokenURI) public {
        ...
        bytes memory baseURI = bytes(baseTokenURI);
        require(baseURI.length > 0, 'baseTokenURI is empty');
        require(baseURI[baseURI.length-1] == '/', 'baseTokenURI should end with "/"');
    ...
    function setTokenURI(uint256 tokenId, string memory customTokenURI) internal {
        require(_exists(tokenId), 'the token does not exist');
        ...
        if (bytes(customTokenURI).length == 0) {
            ...
            _tokenURIs[tokenId] = customTokenURI;
        }
    }
...
contract DFCapstoneTestERC721Metadata is ERC721Metadata {
    ...
    function mint(address to, uint256 tokenId, string memory customTokenURI)
        onlyOwner whenNotPaused
        public
        returns (bool)
    {
        super._mint(to, tokenId);
        setTokenURI(tokenId, customTokenURI);
        return true;
    }
}
